...
from . import charts
from .components import KPI, chips, kpi_row, page_header, panel_title
from .contracts import ENTITY_TYPE_LABELS
...
def _entity_selector(provider: DashboardDataProvider) -> str | None:
    ...
    if not entity_ids:
        ...
        return None
    return col_entity.selectbox("Entity", entity_ids, key="entity_choice")
...
def _render_metadata(summary) -> None:
    ...
    st.markdown(
        ...
        f"Department: `{summary.department or 'unknown'}`  \n"
...
def _render_recent_alerts(provider: DashboardDataProvider, entity_id: str) -> None:
    history = provider.get_entity_alert_history(entity_id, limit=6)
    if history.empty:
...
def render(ctx: DashboardContext) -> None:
    ...
    with right:
        panel_title("Risk activity timeline")
        st.plotly_chart(
